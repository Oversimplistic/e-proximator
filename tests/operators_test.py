...
def test_addition(e):
    approximation, b, c = run_approximator(e, 0, 0)
...
def test_multiplication(e):
    approximation, b, c = run_approximator(e, 1, 0)
    assert approximation == e
    assert b == ["*"]

def test_division(e):
    approximation, b, c = run_approximator(1, e, 0)
    assert approximation == 1
    assert b == ["/"]

def test_power(e_to_the_e, e):
    approximation, b, c = run_approximator(e_to_the_e, e, 0)
    assert approximation == e_to_the_e
    assert b == ["^e"]


def test_root(e_root_e, e):
    approximation, b, c = run_approximator(e_root_e, e, 0)
    assert approximation == e_root_e
    assert b == ["^1/e"]

def test_log(arbitrary_natural_log):
    approximation, b, c = run_approximator(arbitrary_natural_log, 20, 0)
