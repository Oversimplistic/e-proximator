...
def test_addition(e):
    path, approximation, depth, error = beamSearch(e, 0)
...
def test_multiplication(e):
    path, approximation, depth, error = beamSearch(e, 1)
    assert approximation == e
    assert path == ["*"]

def test_division(e):
    path, approximation, depth, error = beamSearch(1, e)
    assert approximation == 1
    assert path == ["/"]

def test_power(e_to_the_e, e):
    path, approximation, depth, error = beamSearch(e_to_the_e, e)
    assert approximation == e_to_the_e
    assert path == ["^e"]


def test_root(e_root_e, e):
    path, approximation, depth, error = beamSearch(e_root_e, e)
    assert approximation == e_root_e
    assert path == ["^1/e"]

def test_log(arbitrary_natural_log):
    path, approximation, depth, error = beamSearch(arbitrary_natural_log, 20)
