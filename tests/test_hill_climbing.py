...
def test_addition(e):
    path, approximation, steps, error = run_hill_climber(e, 0, 0)
...
def test_multiplication(e):
    path, approximation, steps, error = run_hill_climber(e, 1, 0)
    assert approximation == e
    assert path == ["*"]

def test_division(e):
    path, approximation, steps, error = run_hill_climber(1, e, 0)
    assert approximation == 1
    assert path == ["/"]

def test_power(e_to_the_e, e):
    path, approximation, steps, error = run_hill_climber(e_to_the_e, e, 0)
    assert approximation == e_to_the_e
    assert path == ["^e"]


def test_root(e_root_e, e):
    path, approximation, steps, error = run_hill_climber(e_root_e, e, 0)
    assert approximation == e_root_e
    assert path == ["^1/e"]

def test_log(arbitrary_natural_log):
    path, approximation, steps, error = run_hill_climber(arbitrary_natural_log, 20, 0)
